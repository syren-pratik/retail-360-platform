"use strict";(()=>{var e={};e.id=7890,e.ids=[7890],e.modules={20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},54467:(e,t,r)=>{r.r(t),r.d(t,{originalPathname:()=>w,patchFetch:()=>A,requestAsyncStorage:()=>_,routeModule:()=>y,serverHooks:()=>k,staticGenerationAsyncStorage:()=>v});var a={};r.r(a),r.d(a,{POST:()=>f,dynamic:()=>p});var n=r(49303),i=r(88716),o=r(60670),s=r(87070),l=r(27293),c=r(56847);let p="force-dynamic",d=null,u="claude-sonnet-4-5-20250514";try{process.env.ANTHROPIC_API_KEY&&(process.env.AZURE_ENDPOINT?(d=new l.ZP({apiKey:process.env.ANTHROPIC_API_KEY,baseURL:process.env.AZURE_ENDPOINT}),u=process.env.AZURE_MODEL_NAME||"claude-sonnet-4-5"):d=new l.ZP({apiKey:process.env.ANTHROPIC_API_KEY}))}catch{console.warn("Failed to initialize Anthropic client for /api/agents/run")}let m=["promo-scenario","price-strategy","competitive-response","margin-leak","markdown-timing","weekly-briefing","event-readiness","demand-anomaly"];function g(e,t){return"number"==typeof e?`${(e/1e5).toFixed(1)}L`:t}let h=["bar_chart","line_chart","donut_chart","data_table","kpi_card","comparison","text_only"];async function f(e){let t=await e.json(),{agent_id:r}=t;if(!m.includes(r))return s.NextResponse.json({error:`Unknown agent: ${r}`},{status:400});let a=function(e){let t=(e.headers.get("cookie")??"").split(/;\s*/).find(e=>e.startsWith(`${c.iv}=`));return"us_apparel"===t?.split("=")[1]?"us_apparel":"india_grocery"}(e),n=`You are a specialized retail pricing AI agent for ${"us_apparel"===a?"a US apparel retailer (departments Mens/Womens/Kids/Footwear/Accessories; events BTS, BFCM, Holiday)":"an Indian supermarket chain"}. You analyze specific requests and return structured insights.
...
${"us_apparel"===a?"- Use $ values: $9.8K (thousands), $1.2M (millions). NEVER use ₹, lakhs, or crores.":"- Use ₹ values: ₹9.8L (lakhs), ₹1.2Cr (crores)"}
