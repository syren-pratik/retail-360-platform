...
export default function AcquisitionByChannel({ data, deepDiveUrl }: AcquisitionByChannelProps) {
  const { activeDrilldowns, addDrilldown } = useDashboard();
  ...
  const handleExpand = () => {
    if (deepDiveUrl) router.push(deepDiveUrl);
  };
